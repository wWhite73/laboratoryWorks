	...
	void check_diagonal_dominance() {
		for (int i = 0; i < this->N; i++) {
			...
			if (fabs(this->table[i][i]) <= fabs(sum)) {
				...
				throw invalid_argument("не выполняется условие дагонального преобладания");
			}
		}
...
public:
	Matrix(int n, int m) {
		...
		if (n < m - 1) {
			throw invalid_argument("количество уравнений меньше, чем количечтво неизвестных переменных");
		}
